...
const filters = [
  {
    ...
    name: 'Category',
    options: [
      { value: 'Apparel', label: '', checked: false },
      { value: 'Drinkware', label: '', checked: false },
      { value: 'Stationery', label: '', checked: true },
...
export default function MerchFilters({children, header, deptSelect}:Props) {
          ...
          <section aria-labelledby="products-heading" className="pt-6 pb-24">
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
              ...
              <form className="hidden lg:block">
                <h3 className="font-medium text-gray-900 my-3 text-lg ">Categories</h3>
                <ul role="list" className="space-y-4 border-b border-gray-200 pb-6 text-sm font-medium text-gray-900">
                  {subCategories.map((category) => (
                    <li key={category.name}>
                      <button onClick={()=>router.push(category.href)}  className='hover:text-marshall-700'>{category.name}</button>
                    </li>
                  ))}
                </ul>
