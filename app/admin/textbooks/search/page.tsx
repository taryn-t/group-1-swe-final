...
export default function Page() {
    ...
    const [search, setSearch] = useState<Array<TextbookDocument>>()
  ...
  async function getSearch<Textbook>(){
    const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/textbooks/search?q=${query}`, {
    ...
    <div className="grid grid-flow-row items-start justify-items-center min-h-screen h-full  gap-16 sm:p-6 font-[family-name:var(--font-geist-sans)]  overflow-y-scroll  ">
        ...
        <div className="-mt-2 -ml-4 flex flex-wrap items-center justify-between sm:flex-nowrap">
          <div className="mt-2 ml-4">
            <h3 className="text-base font-semibold text-gray-900">{search.length} results for &quot;{query}&quot;</h3>
          </div>
        ...
        {search?.map((search) => (
            <li key={search._id} className="flex items-center justify-between gap-x-6 py-5 overflow-visible ">
            <div className="min-w-0">
                <div className="flex items-start gap-x-3">
                <p className="text-sm/6 font-semibold text-gray-900">{search.name}</p>
                <p
            ...
            <div className="flex flex-none items-center gap-x-4">
                <Link
                href={`/admin/textbooks/${search._id}`}
                ...
                >
                Edit book<span className="sr-only">, {search.name}</span>
                </Link>
                ...
                >
                    <MenuItem>
                    <button
                        onClick={()=>deleteTextbook(search._id)}
